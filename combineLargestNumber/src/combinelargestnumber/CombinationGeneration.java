...
/**
 * Class used for generating the largest possible concatenation of given
 * integers.
 * 
 * @author makpo
 *
...
public class CombinationGeneration {
	...
	private class IntegerComparator implements Comparator<Integer> {
		...
		public int compare(Integer in1, Integer in2) {
			...
			// if the 2 numbers appear to be of the same length, then compare to keep the
			// larger one.
			if (str1.length() == str2.length()) {
				...
				// if they are not, we need to keep the one with the greater 1st digit, no
				// matter the length.
				// To compare, we need to bring them on the same length.
				int max_length = Math.max(str1.length(), str2.length());
				while (str1.length() < max_length * 2)
					str1 += initial1;
				while (str2.length() < max_length * 2)
					str2 += initial2;
	...
	/**
	 * This is a method that generates and returns the largest number combining the
	 * given integers
	 * 
